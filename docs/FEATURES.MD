...
# Easy running of debug-only detekt task

Running debug-only detekt tasks can be cumbersome on the mixed Android/Kotlin project, as just running `detektMain` will
run both debug and release detekt (which are most often duplicates), while running `detektDebug` will skip tests from
non-Android modules.

Gradle config of this project introduces `runDebugDetekt` tasks that only run detekt checks from debug and non-Android flavors
